 ...
 * @param mimeTypes the mimeType"
 * @param action the action"
 */
class DeepLinkPattern<T : NavKey>(
    val serializer: KSerializer<T>,
    val urlPattern: Url?,
    val urlOnlyProtocol: Boolean = false,
    val mimeTypes: List<String> = emptyList(),
    val action: String? = null,
    val typeParsers: Map<SerialKind, TypeParser> = emptyMap(),
) {
    /**
    ...
    val pathSegments: List<PathSegment> = buildList {
        urlPattern?.rawSegments?.forEach { segment ->
            ...
            if (result != null) {
                ...
                if (elementIndex == CompositeDecoder.UNKNOWN_NAME) {
                    throw IllegalArgumentException(
                        "Path parameter '{$argName}' defined in the DeepLink $urlPattern does not exist in the Serializable class '${serializer.descriptor.serialName}'."
                    )
                }
    ...
    val queryValueParsers: Map<String, TypeParser> = buildMap {
        urlPattern?.parameters?.names()?.forEach { paramName ->
            val elementIndex = serializer.descriptor.getElementIndex(paramName)
            if (elementIndex == CompositeDecoder.UNKNOWN_NAME) {
                throw IllegalArgumentException(
                    "Query parameter '$paramName' defined in the DeepLink $urlPattern does not exist in the Serializable class '${serializer.descriptor.serialName}'."
                )
            }
            val elementDescriptor = serializer.descriptor.getElementDescriptor(elementIndex)
            this[paramName] = getTypeParser(elementDescriptor.kind, typeParsers)
        }
    }
